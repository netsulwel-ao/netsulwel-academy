...
import { Plus, Trash2, X, UploadCloud, Link as LinkIcon, Loader2, FileText } from "lucide-react";
        ...
        <div className="space-y-2 pl-4 border-l border-gray-800">
          {materials.length > 0 && (
            <div className="space-y-1">
              {materials.map((mat, i) => (
                <div key={i} className="flex items-center gap-2 bg-gray-900/60 border border-gray-800 px-3 py-2 text-xs text-gray-300 group">
                  <FileText className="h-3.5 w-3.5 text-blue-400 shrink-0" />
                  <span className="truncate flex-1">{mat.title || mat.filename}</span>
                  <span className="text-gray-600 shrink-0 uppercase text-[10px]">{mat.type}</span>
          ...
          <div className="flex items-center gap-2">
            <input type="text" value={title} onChange={e => setTitle(e.target.value)}
              ...
              className="flex-1 bg-gray-900 border border-gray-800 px-2.5 py-1.5 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-blue-500/50" />
            <select value={type} onChange={e => setType(e.target.value as MaterialType)}
              className="w-24 bg-gray-900 border border-gray-800 px-2 py-1.5 text-xs text-white focus:outline-none">
          ...
          <div className="flex items-start gap-2">
            <div className="flex border border-gray-800 overflow-hidden shrink-0">
              <button type="button" onClick={() => setUrlMode("link")}
                className={`px-2.5 py-1.5 text-[10px] font-medium transition-colors ${urlMode === "link" ? "bg-purple text-white" : "bg-gray-900 text-gray-500 hover:text-gray-300"}`}>
                <LinkIcon className="w-3 h-3" />
              </button>
              <button type="button" onClick={() => setUrlMode("upload")}
                className={`px-2.5 py-1.5 text-[10px] font-medium transition-colors ${urlMode === "upload" ? "bg-purple text-white" : "bg-gray-900 text-gray-500 hover:text-gray-300"}`}>
            ...
            {urlMode === "link" ? (
              <div className="flex-1 flex gap-1">
                ...
                <button type="button" onClick={addLink} disabled={!url.trim()}
                  className="px-2.5 py-1.5 bg-purple text-white text-xs font-medium disabled:opacity-40 flex items-center gap-1">
              ...
              <div className="flex-1">
                <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading}
                  className="w-full flex items-center justify-center gap-1.5 border border-dashed border-gray-700 hover:border-blue-500/50 bg-gray-900 py-2 text-[10px] text-gray-400 hover:text-blue-400 transition-colors">
