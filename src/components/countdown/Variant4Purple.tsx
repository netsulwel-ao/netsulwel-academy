  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-6 bg-gradient-to-r from-[#2d0a4d] to-[#a020f0] px-8 py-8">
      <div className="text-center sm:text-left">
        ...
        <p className="text-sm text-white/70 mt-1">Garanta o seu acesso antes que termine</p>
      </div>
      <div className="flex items-center gap-3">
        {units.map((u, i) => (
          <div key={u.label} className="flex flex-col items-center justify-center w-16 h-20 bg-white/10 border border-white/20 backdrop-blur-sm rounded-lg">
            <span className="text-2xl font-bold text-white tabular-nums">
              {String(u.value).padStart(2, "0")}
            </span>
            <span className="text-[10px] text-white/60 mt-0.5">{u.label}</span>
          </div>
        ))}
      </div>
      {ctaUrl ? (
        <Link href={ctaUrl}
          className="bg-white text-[#a020f0] hover:bg-white/90 font-bold px-6 py-3 rounded-lg transition-colors whitespace-nowrap">
