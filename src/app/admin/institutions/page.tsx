    ...
    <div className="max-w-[80rem] mx-auto space-y-8 animate-in fade-in duration-300">
          ...
          {(["pending", "approved", "suspended"] as const).map(s => {
            ...
            return (
              <button
                ...
                className={`border border-gray-800/60 bg-gray-900/10 px-4 py-3 text-left transition-all ${filterStatus === s ? "ring-1 ring-inset ring-purple/20" : "hover:bg-gray-900/20"}`}
              >
                <div className="flex items-center gap-2 mb-1">
                  <span className={`h-1.5 w-1.5 shrink-0 ${dot}`} />
                  <span className={`font-mono text-[9px] uppercase tracking-widest ${cls.split(" ")[1]}`}>{label}</span>
                </div>
        ...
        {showFilters && (
          <div className="flex flex-wrap items-center gap-2 border border-gray-800/40 bg-gray-900/10 px-4 py-3">
            <select
              ...
              className="border border-gray-800/60 bg-gray-900 text-xs text-gray-400 px-3 py-1.5 focus:outline-none focus:border-purple/30"
            >
            ...
            {hasFilters && (
              <button onClick={clearFilters} className="font-mono text-[9px] uppercase tracking-widest text-gray-600 hover:text-gray-400 flex items-center gap-1 transition-colors">
      ...
      {!loading && filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center border border-gray-800/60 bg-gray-900/10 py-20 text-center">
          ...
          {hasFilters && (
            <button onClick={clearFilters} className="mt-4 font-mono text-[10px] uppercase tracking-widest text-gray-600 hover:text-gray-400 transition-colors">
              ← Limpar filtros
            </button>
          )}
        </div>
      ...
      {!loading && filtered.length > 0 && (
        <div className="border border-gray-800/60">
          <div className="hidden lg:grid grid-cols-[2fr_1fr_1fr_1fr_auto] gap-4 px-5 py-3 border-b border-gray-800/40 bg-gray-900/20">
            <p className="font-mono text-[9px] uppercase tracking-widest text-gray-700">Instituição</p>
            ...
            <p className="font-mono text-[9px] uppercase tracking-widest text-gray-700">Registada</p>
          ...
          <div className="divide-y divide-gray-800/30">
            {filtered.map(inst => (
              <div
                key={inst.id}
                className="grid grid-cols-1 lg:grid-cols-[2fr_1fr_1fr_1fr_auto] gap-3 items-center px-5 py-4 hover:bg-gray-900/20 transition-colors"
              >
                {/* Nome */}
                <div className="flex items-center gap-3 min-w-0">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center border border-gray-800/60 bg-gray-900 font-semibold text-xs text-gray-400">
                    {inst.name.charAt(0).toUpperCase()}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-gray-200 truncate">{inst.name}</p>
                    <p className="font-mono text-[9px] text-gray-700 truncate">{inst.id.slice(0, 12)}…</p>
                  </div>
                </div>
                ...
                <div className="hidden lg:flex items-center gap-1.5">
                  ...
                  <span className="font-mono text-xs text-gray-500 truncate">{inst.email}</span>
                </div>
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  {inst.status === "pending" && (
                    <button
                      ...
                      className="flex items-center gap-1 border border-green/25 bg-green/8 px-3 py-1.5 font-mono text-[9px] uppercase tracking-widest text-green/80 hover:bg-green/15 disabled:opacity-40 transition-all"
                    >
                      {actioningId === inst.id
                  ...
                  {inst.status === "approved" && (
                    <button
                      ...
                      disabled={actioningId === inst.id}
                      className="flex items-center gap-1 border border-red-500/25 bg-red-500/8 px-3 py-1.5 font-mono text-[9px] uppercase tracking-widest text-red-400/80 hover:bg-red-500/15 disabled:opacity-40 transition-all"
                    >
                      {actioningId === inst.id
                        ? <Loader2 className="h-3 w-3 animate-spin" />
      ...
      {selectedInst && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-sm bg-gray-950 border-l border-gray-800/60 p-6 overflow-y-auto animate-in slide-in-from-right duration-200">

            <div className="flex items-center justify-between mb-6">
              <p className="font-mono text-[10px] uppercase tracking-widest text-gray-700">// detalhes</p>
            ...
            {/* Identidade */}
            <div className="flex flex-col items-center gap-3 mb-6 pb-6 border-b border-gray-800/60 text-center">
            ...
            <div className="space-y-2">
              {selectedInst.status === "pending" && (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-1.5 border border-green/30 bg-green/8 py-2.5 font-mono text-[10px] uppercase tracking-widest text-green/80 hover:bg-green/15 disabled:opacity-40 transition-all"
                >
              ...
              {selectedInst.status === "approved" && (
                <button
                  ...
                  disabled={actioningId === selectedInst.id}
                  className="w-full flex items-center justify-center gap-1.5 border border-red-500/30 bg-red-500/8 py-2.5 font-mono text-[10px] uppercase tracking-widest text-red-400/80 hover:bg-red-500/15 disabled:opacity-40 transition-all"
                >
              ...
              <Link
                ...
                className="w-full flex items-center justify-center gap-1.5 border border-gray-800/60 bg-gray-900/10 py-2.5 font-mono text-[10px] uppercase tracking-widest text-gray-500 hover:border-gray-700 hover:text-gray-300 transition-all"
              >
