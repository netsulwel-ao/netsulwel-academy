    ...
    <div className="max-w-[80rem] mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      {!loading && (lives.length > 0 || requests.length > 0) && (
        <div className="space-y-8">
          ...
          {active.length > 0 && (
            <section>
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="font-mono text-[10px] uppercase tracking-widest text-red-400/70">ao vivo agora</p>
              </div>
              <div className="border border-red-500/20 divide-y divide-gray-800/40">
                {active.map(l => (
                  <div key={l.id} className="flex flex-col sm:flex-row sm:items-center gap-4 px-5 py-4 bg-red-500/5">
                    <div className="flex-1 min-w-0">
                      ...
                      {l.description && <p className="text-xs text-gray-600 mt-0.5 truncate">{l.description}</p>}
                    </div>
          ...
          {upcoming.length > 0 && (
            <section>
              <p className="font-mono text-[10px] uppercase tracking-widest text-gray-700 mb-3">// agendadas · {upcoming.length}</p>
              <div className="border border-gray-800/60 divide-y divide-gray-800/40">
                {upcoming.map(l => (
                  <div key={l.id} className="flex flex-col sm:flex-row sm:items-center gap-4 px-5 py-4 hover:bg-gray-900/20 transition-colors">
                    <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <Link href={`/dashboard/teacher/lives/${l.id}/studio`}
                        className="flex items-center gap-1.5 bg-green px-3 py-2 text-xs font-bold text-gray-950 hover:bg-green-light transition-colors">
                        <Play className="h-3.5 w-3.5" strokeWidth={1.5} /> Iniciar
                      </Link>
                      <Link href={`/dashboard/teacher/lives/${l.id}/edit`}
                        className="flex h-8 w-8 items-center justify-center border border-gray-800 bg-gray-900/60 text-gray-600 hover:border-gray-700 hover:text-gray-400 transition-all">
          ...
          {past.length > 0 && (
            <section>
              <p className="font-mono text-[10px] uppercase tracking-widest text-gray-700 mb-3">// realizadas · {past.length}</p>
              <div className="border border-gray-800/60 divide-y divide-gray-800/40">
                {past.map(l => (
                  <div key={l.id} className="flex items-center justify-between gap-4 px-5 py-3.5 opacity-60 hover:opacity-80 transition-opacity">
                    <div className="min-w-0">
                      ...
                      <p className="font-mono text-[9px] text-gray-700">{fmtDateTime(l.scheduledAt)}</p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="font-mono text-[9px] uppercase tracking-widest text-gray-700">concluída</span>
                      <button type="button" onClick={() => handleDelete(l.id!, l.title)} disabled={deletingId === l.id}
          ...
          {requests.length > 0 && (
            <section>
              <p className="font-mono text-[10px] uppercase tracking-widest text-gray-700 mb-3">// pedidos de lives gratuitas · {requests.length}</p>
              <div className="border border-gray-800/60 divide-y divide-gray-800/40">
                {requests.map(r => (
                  <div key={r.id} className="flex flex-col sm:flex-row sm:items-center gap-3 px-5 py-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-3 mt-1">
                        {r.scheduledAt && (
                          <p className="font-mono text-[9px] text-gray-700 flex items-center gap-1">
                            <Calendar className="h-2.5 w-2.5" strokeWidth={1.5} />
                            {new Date(r.scheduledAt).toLocaleDateString("pt-PT")}
                          </p>
                        )}
                        {r.status === "approved" && (
                          <p className="font-mono text-[9px] text-green/60">
                    ...
                    <span className={`shrink-0 flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-widest border px-2.5 py-1 ${
                      r.status === "approved"
              ...
              {/* Nota informativa */}
              <div className="mt-3 border border-gray-800/40 bg-gray-900/10 px-4 py-3">
