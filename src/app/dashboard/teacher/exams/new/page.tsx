...
const inputCls = "w-full border border-gray-800 bg-gray-900/60 px-3 py-2.5 text-sm text-gray-200 placeholder-gray-700 focus:border-green/40 focus:outline-none transition-colors";
const labelCls = "font-mono text-[9px] uppercase tracking-widest text-gray-700 mb-1.5 block";
    ...
    <div className="max-w-[56rem] mx-auto space-y-6 animate-in fade-in duration-300">
      ...
      <div>
        <button
          ...
          className="flex items-center gap-1 font-mono text-[10px] uppercase tracking-widest text-gray-700 hover:text-gray-500 transition-colors mb-4"
        >
      ...
      <div className="border border-gray-800/60 bg-gray-900/20 p-5 space-y-4">
        <p className="font-mono text-[10px] uppercase tracking-widest text-gray-700">// configuração</p>
            ...
            <select value={courseId} onChange={e => setCourseId(e.target.value)}
              ...
              {courses.map(c => (
                <option key={c.id} value={c.id} className="bg-gray-900">
                  {c.title} {c.type === "smart" ? "· Smart" : c.type === "golden" ? "· Golden" : c.price > 0 ? `· ${c.price.toLocaleString("pt-AO")} Kz` : "· Grátis"}
                </option>
              ))}
            </select>
            {courses.length === 0 && (
            ...
            <select value={liveId} onChange={e => setLiveId(e.target.value)}
              ...
              {lives.map(l => (
                <option key={l.id} value={l.id} className="bg-gray-900">
                  {l.title} {l.target === "free" ? "· Grátis" : l.target === "smart" ? "· Smart" : l.target === "golden" ? "· Golden" : l.price ? `· ${l.price.toLocaleString("pt-AO")} Kz` : ""}
                </option>
              ))}
            </select>
            {lives.length === 0 && (
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={addQuestion}
            className="flex items-center gap-1.5 border border-green/25 bg-green/8 px-3 py-1.5 font-mono text-[10px] uppercase tracking-widest text-green/70 hover:bg-green/15 transition-all"
          >
        ...
        {questions.map((q, i) => (
          <div key={q.id} className="border border-gray-800/60 bg-gray-900/20">
            {/* Header da pergunta */}
            <div className="flex items-center justify-between border-b border-gray-800/40 px-4 py-2.5">
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  className="border border-gray-800 bg-gray-900/60 px-2 py-1 font-mono text-[9px] uppercase tracking-widest text-gray-500 focus:outline-none appearance-none"
                >
            ...
            <div className="p-4 space-y-3">
              ...
              {q.type === "multiple_choice" && q.options && (
                <div className="space-y-2">
                  ...
                  <p className="font-mono text-[9px] text-gray-700">Selecciona o círculo da opção correcta.</p>
                </div>
              )}
