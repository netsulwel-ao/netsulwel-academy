  ...
  return (
    <div className="max-w-[100rem] space-y-8 animate-in fade-in duration-500">
        ...
        <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <div>
              ...
              <p className="text-sm sm:text-base text-gray-400">Publica anúncios para os membros da instituição</p>
            </div>
          </div>
          <button onClick={() => setShowCreate(true)}
            ...
            <Plus className="h-4 w-4 sm:h-5 sm:w-5" />Novo Comunicado
          </button>
        </div>
      </div>
      ...
      {announcements.length === 0 ? (
        ...
        <div className="space-y-4">
          {announcements.map(a => (
            <div key={a.id} className="group relative overflow-hidden rounded-xl bg-gradient-to-br from-gray-900/60 to-gray-950/60 border border-gray-800/70 hover:border-purple/30 transition-all duration-300 p-4 sm:p-6">
              <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-purple-500 to-purple-700 rounded-l" />
              <div className="relative flex items-start gap-3 sm:gap-4">
                <div className="h-8 w-8 sm:h-10 sm:w-10 rounded-lg bg-purple-500/10 flex items-center justify-center shrink-0 mt-0.5">
                  <Megaphone className="h-4 w-4 sm:h-5 sm:w-5 text-purple-400" />
                </div>
