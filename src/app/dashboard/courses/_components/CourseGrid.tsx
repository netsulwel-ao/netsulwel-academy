...
import { normalizeCourseType } from "../_types/catalog";
...
export function CourseGrid({
  ...
  const { requiredPlanLabel } = useAccess();

  // ── Empty state ──────────────────────────────────────────────
  if (accessible.length === 0 && locked.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 border border-gray-800/60 bg-gray-900/10 text-center">
        ...
        {hasActiveFilters && (
          <button
            ...
            className="mt-4 border border-gray-800 bg-gray-900/60 px-4 py-2 font-mono text-[10px] uppercase tracking-widest text-gray-600 hover:border-gray-700 hover:text-gray-400 transition-colors"
          >
            Limpar filtros
          </button>
        )}
      ...
      {accessible.length > 0 && (
        <section>
          ...
          <div className="flex items-end justify-between mb-4">
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[0.25em] text-green/60 mb-1">
                // disponíveis
              </p>
              <h2 className="text-base font-bold text-gray-200">
                Cursos acessíveis
                <span className="ml-2 font-mono text-xs text-gray-700">({accessible.length})</span>
              </h2>
            </div>
            <Link
              href="/dashboard/courses"
              className="font-mono text-[10px] uppercase tracking-widest text-gray-700 hover:text-gray-500 transition-colors"
            >
              ver todos →
            </Link>
          </div>
          ...
          {accessible.length === 1 ? (
            ...
            <div className="space-y-3">
              ...
              <Link
                ...
                className="group relative flex flex-col md:flex-row overflow-hidden border border-gray-800/60 bg-gray-900/20 hover:border-gray-700 hover:bg-gray-900/40 transition-all"
              >
                <div className="relative h-44 md:h-auto md:w-72 lg:w-80 shrink-0 overflow-hidden bg-gray-900">
                  ...
                  {ownCourseIds.has(accessible[0].id!) && (
                    <div className="absolute left-2.5 top-2.5">
                      <span className="font-mono text-[9px] uppercase tracking-widest border border-purple/30 bg-purple/15 px-2 py-0.5 text-purple/80">
                        Teu curso
                      </span>
                    </div>
                  )}
                ...
                <div className="flex flex-1 flex-col justify-between p-4 sm:p-5">
                  <div>
                    <p className="font-mono text-[9px] uppercase tracking-widest text-gray-700 mb-2">// destaque</p>
                  ...
                  <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-800/40">
                    <span className="font-mono text-[11px] text-gray-700">{accessible[0].lessonsCount ?? 0} aulas</span>
                    <span className="flex items-center gap-1 font-mono text-[11px] text-purple/60 group-hover:text-purple/80 transition-colors">
          ...
          <div className="flex items-end justify-between mb-4">
            <div>
              ...
              <h2 className="text-base font-bold text-gray-500">
                Cursos bloqueados
                <span className="ml-2 font-mono text-xs text-gray-700">({locked.length})</span>
              </h2>
            </div>
            <Link
              href="#planos"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {locked.map(c => (
              <CourseCard
                ...
                locked={true}
                enrolledCourses={enrolledCourses}
                requiredPlan={requiredPlanLabel(normalizeCourseType(c.type))}
                creatorNames={creatorNames}
              />
            ))}
          </div>
