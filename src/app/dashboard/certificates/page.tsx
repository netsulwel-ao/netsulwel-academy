        ...
        <div className="space-y-2">
          {certs.map((cert, i) => {
            ...
            return (
              ...
              >
                {/* Ícone */}
                <div className={`shrink-0 flex items-center justify-center border border-amber-500/20 bg-amber-500/8 group-hover:bg-amber-500/15 transition-colors ${
                  isFirst ? "h-12 w-12" : "h-9 w-9"
                }`}>
                  <Award className={`text-amber-400/70 ${isFirst ? "h-6 w-6" : "h-4 w-4"}`} strokeWidth={1.5} />
                </div>
                ...
                {/* Acção */}
                <ChevronRight className="h-4 w-4 text-gray-700 group-hover:text-amber-400/60 shrink-0 transition-colors" />
              </Link>
            );
          })}
        </div>
